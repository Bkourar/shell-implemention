...
t_exp	*update_input(char *in, t_exp **lst, bool b)
{
	...
	while (in[i])
	{
		...
		if (valid_condtion(in, i))
			(copy_input(stack, in, &i, &st, lst), upgrade_type(&tp, true));
		else if (in[i] == '\'' &&  in[i + 1] != '\'')
			(copy_whit_sq(stack, in, &i, &st, lst), upgrade_type(&tp, false));
		else if (in[i] == '\"' && in[i + 1] != '$' && in[i + 1] != '\"')
			(copy_whit_dq(stack, in, &i, &st, lst), upgrade_type(&tp, false));
		else
			(copy_standard(stack, in, &i, &st, lst) ,upgrade_type(&tp, false));
