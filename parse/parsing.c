...
void	pi_processing_err_1(t_sh **f, t_sh **l, char *del)
{
	t_sh	*t;
	...
	while (t->nx != NULL)
	{
		if (t->type == Pipe && t->nx->type == Pipe && !check_pos_here(f, &t->nx))
			...
			(run_heredoc(del, 0), synatx_error("|"));//here run heredoc && run err
		t = t->nx;
	}
}
	...
	while (tp1 != NULL)
	{
		if (check_op1(tp->type) && check_op1(tp1->type))
		{
			if (!check_pos_here(f, &tp1))
				...
				(run_heredoc(del, 0), synatx_error(tp1->token));//here run heredoc && err
		}
...
void	pi_processing_err_3(t_sh **l, t_sh **ps, char c0, char c1, char *del)
{
	if (c1 == '>' && !check_pos_here(l, ps))
		...
		(run_heredoc(del, 0), synatx_error(">"));//run here_doc && err
	else if (c1 == '<' && check_pos_here(l, ps))
		(run_heredoc(del, 0), synatx_error("<"));//run here_doc && err
		...
		(run_heredoc(del, 0), synatx_error(">"));//run heredoc &&  err
	else if (c0 == '|' && c1 == '<' && check_pos_here(l, ps))
		(run_heredoc(del, 0), synatx_error("<"));//run heredoc &&  err
	else if (c1 == '|' && !check_pos_here(l, ps))
		(synatx_error("|"));//run just err
