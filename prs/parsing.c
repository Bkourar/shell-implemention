...
int	pi_processing_err_1(t_sh **f, t_sh **l, char *del)
{
	...
	if ((*f)->type == Pipe || ((*f)->nx == NULL && check_op1((*f)->type)))
		...
		return (run_heredoc(del, 0, f, NULL), synatx_error((*l)->token), synatx_error("3535353"),1);//here run herdoc && errors
		...
		&& !check_herepipe(f))
		return (run_heredoc(del, 0, f, NULL), print_errors(), puts("hereee"), 1);//here run herdoc && error
	else if (check_op1((*l)->type) && !check_op1((*l)->pv->type) 
		&& check_herepipe(f) != 0)
	...
	while (t->nx != NULL)
	{
		if (t->type == Pipe && t->nx->type == Pipe && !check_pos_here(f, &t->nx))
			...
			return (run_heredoc(del, 0, f, NULL), synatx_error("|21212121"), 1);//here run heredoc && run err
		t = t->nx;
	}
	return (0);
}
	...
	while (tp1 != NULL)
	{
		if (check_op1(tp->type) && check_op1(tp1->type))
		{
			if (!check_pos_here(f, &tp1))
				...
				return (run_heredoc(del, 0, f, NULL), synatx_error(tp1->token), synatx_error("3333"),1);//here run heredoc && err
		}
		else if (check_op1(tp->type) && tp1->type == Pipe)
		{
			if (!check_pos_here(f, &tp1))
				...
				return (run_heredoc(del, 0, f, NULL), synatx_error(tp1->token), synatx_error("777"),1);//here run heredoc && err
		}
		tp = tp1;
		tp1 = tp1->nx;
	}
...
int	pi_processing_err_3(t_sh **l, t_sh **ps, char c0, char c1, char *del)
{
	if (c1 == '>' && !check_pos_here(l, ps))
		...
		return (run_heredoc(del, 0, l, NULL), synatx_error(">3"), 1);//run here_doc && err
	else if (c1 == '<' && check_pos_here(l, ps))
		return (run_heredoc(del, 0, l, NULL), synatx_error("<4"), 1);//run here_doc && err
	else if (c0 == '|' && c1 == '>' && !check_pos_here(l, ps))
		return (synatx_error(">5"), 1);//run just err
	else if (c0 == '|' && c1 == '<' && !check_pos_here(l, ps))
		return (synatx_error("<6"), 1);//run just err
	else if (c0 == '|' && c1 == '>' && check_pos_here(l, ps))
		return (run_heredoc(del, 0, l, NULL), synatx_error(">7"), 1);//run heredoc &&  err
	else if (c0 == '|' && c1 == '<' && check_pos_here(l, ps))
		return (run_heredoc(del, 0, l, NULL), synatx_error("<8"), 1);//run heredoc &&  err
	else if (c1 == '|' && !check_pos_here(l, ps))
		return (synatx_error("|9"), 1);//run just err
	else if (c1 == '|' && check_pos_here(l, ps))
		return (run_heredoc(del, 0, l, NULL), synatx_error("|10"), 1);//run here_doc && err
	return (0);
}
